...
Connection.prototype.setOptions = function (options) {
  ...
  var sslo =
    options && options.ssl && options.ssl.enabled ? defaultSslOptions : {};
  this.options = _.assignIn({}, defaultOptions, sslo, urlo, options || {});
  this.options.clientProperties = _.assignIn(
    {},
    defaultClientProperties,
    (options && options.clientProperties) || {}
  );
};
...
Connection.prototype.addAllListeners = function () {
  ...
  self.addListener(connectEvent, function () {
    ...
    self.parser.onContentHeader = function (
      channel,
      classInfo,
      weight,
      properties,
      ...
      debug &&
        debug(
          channel +
            " > content header " +
            JSON.stringify([classInfo.name, weight, properties, size])
        );
      if (self.channels[channel] && self.channels[channel]._onContentHeader) {
        self.channels[channel]._onContentHeader(
          channel,
          classInfo,
          weight,
          properties,
    ...
    if (!self.connectionAttemptScheduled) {
      ...
      if (self.implOptions.reconnect) {
        ...
        if (backoffTime === null) {
          ...
          self.implOptions.reconnectBackoffStrategy === "exponential"
        ) {
  ...
  if (this.options.heartbeat) {
    ...
    this._inboundHeartbeatTimer = setTimeout(function () {
      if (self.socket.readable || self.options.heartbeatForceReconnect) {
        self.emit(
          "error",
          new Error("no heartbeat or data in last " + gracePeriod + " seconds")
        );
      }
    }, gracePeriod * 1000);
  }
...
Connection.prototype._saslResponse = function () {
  ...
  else if (this.options.authMechanism == "EXTERNAL") response = "\0";
  else if (this.options.authMechanism == "ANONYMOUS") response = "\0";
  else response = this.options.response;
...
Connection.prototype._onMethod = function (channel, method, args) {
  ...
  if (channel > 0) {
    ...
    if (!this.channels[channel]._onChannelMethod) {
      throw new Error(
        "Channel " + channel + " has no _onChannelMethod method."
      );
    }
  ...
  switch (method) {
    ...
    case methods.connectionBlocked:
      debug &&
        debug(
          "Received connection.blocked from server with reason: " + args.reason
        );
      this._blocked = true;
...
Connection.prototype._parseURLOptions = function (connectionString) {
  ...
  if (scheme != "amqp" && scheme != "amqps") {
    throw new Error(
      "Connection URI must use amqp or amqps scheme. " +
  ...
  if (Array.isArray(this.options.host)) {
    if (this.hosti == null) {
      if (typeof this.options.hostPreference == "number") {
        this.hosti =
          this.options.hostPreference < this.options.host.length
            ? this.options.hostPreference
            : this.options.host.length - 1;
      } else {
...
Connection.prototype._getSSLOptions = function () {
  ...
  this.sslConnectionOptions.rejectUnauthorized =
    this.options.ssl.rejectUnauthorized;
